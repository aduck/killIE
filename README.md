...
自定义设置：hr{width:90%;margin:0 auto;height:1px;border:none;background:#ccc;color:#ccc;} /*设置color为了兼容IE*/

> 可能会有默认margin，慎用

处理方法：hr{width:90%;margin:0 auto;height:1px;background:#ccc;color:#ccc;*margin-bottom:-14px;*float:left;}
...
scrollTop兼容性问题
  ...
  问题描述：在IE和FF中，document.body.scrollTop总为0，在chrome中document.documentElement.scrollTop总为0；
...
做到IE67兼容设置div *font-size:0.873*容器高度
...
⑤type为需要返回的数据类型，只有返回类型为json时需要加上，此外期待返回类型为xml时需要服务器设置content-type类型，header("Content-Type:text/xml;charset=utf-8");
例：
$('#btn').click(function(){
	$.get('get1.php',{
